...
public abstract class SubclassCreatorTemplate extends CreatorTemplate {
    ...
    private static Class<?>[] toNonInterfaces(Class<?>[] proxyClasses) {
        Set<Class<?>> superclasses = CollectionUtil.createLinkedHashSet();
        ...
        switch (superclasses.length) {
            ...
            case 1:
                ...
                if (Modifier.isFinal(superclass.getModifiers())) {
                    throw new ProxyCreatorException("Proxy class cannot extend {} as it is final.",
                            superclass.getName());
                }
                if (!hasDefaultConstructor(superclass)) {
                    throw new ProxyCreatorException(
                            "Proxy class cannot extend {}, because it has no visible \"default\" constructor.",
                            superclass.getName());
                }
