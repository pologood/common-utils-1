...
import static com.baidu.unbiz.common.ClassUtil.getSimpleMethodSignature;
 ...
 * </ol>
 * <p/>
 * 用途示例：Servlet 3.0和Servlet 2.5有些许API的不同。例如：Servlet 3.0的方法：
 * <code>Set&lt;String&gt; ServletContext.getResourcePaths(String path)</code>中使用了generic类型，而早期的API则没有使用generic。
 * 假如你需要扩展并覆盖这个<code>ServletContext</code>的方法，并希望这个代码能在不同的Servlet API版本下编译通过。如果不使用动态类生成技术，就无法兼容两个Servlet API的版本。
 * 
 * 
 * @author <a href="mailto:xuchen06@baidu.com">xuc</a>
 * @version create on 2016年3月20日 上午2:43:06
 */
public class InterfaceImplementorBuilder extends DynamicClassBuilder {
    private static final Callback CALL_SUPER = new CallSuper();
        ...
        if (methods != null) {
            for (Method overriderMethod : methods) {
                ...
                for (int i = 0; i < overriderMethod.getParameterTypes().length; i++) {
                    if (!overriderMethod.getParameterTypes()[i]
                            .isAssignableFrom(interfaceMethod.getParameterTypes()[i])) {
                        compatible = false;
                        break;
                    }
                }
    ...
    private void init(Object overrider, Object baseObject) {
        ...
        if (baseClass != null) {
            for (Class<?> interfaceClass : interfaces) {
                assertTrue(interfaceClass.isAssignableFrom(baseClass), "Base class %s must implement %s",
                        baseClass.getName(), interfaceClass);
            }
        }
        ...
        generator.setCallbackTypes(new Class<?>[] { CallSuper.class, CallBaseObject.class, CallOverrider.class });
        ...
        generator.setCallbacks(new Callback[] { CALL_SUPER, new CallBaseObject(new Object()),
                new CallOverrider(new Object()) });
    ...
    public Object toObject(Object overrider, Object baseObject) {
        ...
        Object proxy =
                factory.newInstance(new Callback[] { CALL_SUPER, new CallBaseObject(baseObject),
        ...
        if (overriderSetProxyObjectMethod != null) {
            try {
                ...
                throw new RuntimeException("Failed to call "
                        + getSimpleMethodSignature(overriderSetProxyObjectMethod.getJavaMethod(), true),
